...
│   ├── geographic.py                  ← site coordinate and elevation loader
│   ├── setup_options.py               ← interactive system configuration widget
│   ├── resources.py                   ← techno-economic parameter loader
│   ├── timeseries.py                  ← 8760-hour demand and generation profiles
│   ├── model.py                       ← PyPSA network builder, solver, and exporters
...
> **Non-flexible dispatch:** Biomass, Biogas, Geothermal, and WTE are constrained via a linopy lower-bound injected through PyPSA's `extra_functionality` hook, pinning dispatch to exactly `profile(t) × p_nom_opt` each hour.

### Balancing Technologies

| Technology | Dispatch mode | Description |
|---|---|---|
| **Natural Gas** | Flexible backup | Fast-response peaking unit; high variable cost and emissions; subject to carbon tax |
| **Biodiesel** | Flexible backup | Renewable liquid fuel backup; similar dispatch role to natural gas |
| **Biomethane** | Flexible backup | Upgraded biogas; low-emission gas backup; subject to carbon tax based on `CO2_per_MWh` |
| **Synthetic Methane** | Flexible backup | Power-to-gas derived methane; near-zero direct emissions |
| **Grid** | Import/export | External grid connection for systems with network access; import costs apply; excluded from carbon tax |
...
Three objectives are selectable from the interactive setup widget:

| Objective | Description |
|---|---|
| **Lowest LCOE** | Minimises total annualised system cost (capital + O&M + fuel + carbon tax + soft penalties) |
| **Lowest CO₂** | Adds a carbon shadow price of 20,000 currency/tCO₂ on top of the standard carbon tax, heavily penalising emitting technologies |
| **Most Diversified** | Minimises total system cost while requiring every selected technology to install at least `DIVERSIFIED_MIN_MW` (default 1 MW) |
...
- **Non-flexible dispatch constraint** — Biomass, Biogas, Geothermal, and WTE dispatch is pinned to `profile(t) × p_nom_opt` via a custom linopy lower-bound injected through `extra_functionality`
- **Grid loss factor** — raw demand inflated by a configurable distribution loss factor (default 4%)
- **Carbon tax** — a per-tonne CO₂ levy applied to all emitting generators (excluding grid imports); rate configurable in `constants.py`; reflected in marginal costs, total cost, and both export formats
- **Capital Recovery Factor** — CAPEX annualised over asset lifetime at user-defined discount rate
- **Storage round-trip efficiency** — target RTE is split symmetrically per direction: `efficiency_store = efficiency_dispatch = sqrt(RTE_target)`
- **Storage SoC tracking** — cyclic state-of-charge with round-trip efficiency losses; minimum SoC floor enforced via `extra_functionality` hook
- **Storage duration limits** — user-defined maximum energy-to-power ratio per technology
- **Soft curtailment penalty** — small currency/MWh penalty on VRE output to discourage oversizing with curtailment
...
- **Grid connection support** — Grid selectable as a balancing technology; carbon tax is not levied on grid imports
- **Grid congestion analysis** — hourly bus utilisation ratio computed as gross demand / available supply; hours above configurable threshold flagged; monthly breakdown and duration curve produced
...
- **Configurable currency symbol** — accepts any string (€, $, £, RM, Rp, etc.); applied to all cost labels and export files
- **Demand scaling** — multiply demand CSV by a constant factor from the widget without editing source data
- **Three export formats** — hourly Excel (`.xlsx`), structured dashboard JSON (`.json`), and flat economic CSV (`.csv`) for downstream analysis
...
$$c_s^{cap} = \left(K_s^{MW} + K_s^{MWh} \cdot MaxHours_s\right) \cdot CRF_s + OM_s$$

The marginal cost for dispatchable generators includes a carbon tax component:

$$c_g^{mc} = \frac{FuelCost_g}{Efficiency_g} + CO_{2,g} \times CARBON\_TAX \quad \forall g \notin \{\text{Grid}\}$$

### Objective — Lowest CO₂

Marginal cost is augmented with a carbon shadow price (CSP = 20,000 currency/tCO₂) on top of the standard carbon tax:
...
| SoC minimum | $\text{SoC}_{t} \geq SOC\_MIN\_FRACTION \times p^{nom} \times MaxHours$ |
| SoC maximum | $\text{SoC}_{t} \leq p^{nom} \times MaxHours$ |
| Cyclic SoC | Initial SoC = final SoC (enforced via `cyclic_state_of_charge = True`) |
| Non-flexible floor | $p_{g,t} \geq profile(t) \times p^{nom}_g \;\; \forall g \in \{$Biomass, Biogas, Geothermal, WTE$\}$ |
| Grid loss factor | $P_{load}(t) = P_{demand}(t) \times (1 + GLF)$ |
...
| `STORAGE_CHARGE_COST` | 2 | Currency/MWh proxy cost to prevent unnecessary storage cycling |
| `CARBON_SHADOW_PRICE` | 20,000 | Currency/tCO₂ shadow price in Lowest CO₂ mode |
...
| `CARBON_TAX` | 55 | Currency/tCO₂ carbon levy applied to all emitting generators (excluding grid imports) |

The **grid loss factor** inflates raw demand to gross generation requirement: $P_{gross}(t) = P_{demand}(t) \times (1 + GLF)$. A 4% loss factor is typical for small isolated island networks.
...
The **carbon shadow price** in Lowest CO₂ mode is applied on top of the standard carbon tax, forcing the solver to minimise emissions while respecting investment feasibility. The **diversification minimum** ensures no technology is excluded from the portfolio in Most Diversified mode.

The **carbon tax** is applied as a per-MWh cost addition to all dispatchable emitting generators (Natural Gas, Biodiesel, and any technology with non-zero `CO2_per_MWh`). Grid imports are explicitly excluded. The total annual carbon tax liability is computed and exported in both JSON outputs.
...
| `Project_Description` | string | Optional free-text description embedded in JSON export |
...
| `Efficiency` | 0–1 | Generator thermal efficiency or storage round-trip efficiency |
...
| `brand_technologies` | string | Optional technology/model specification embedded in JSON export |

**Note:** `Fuel_Cost` is per MWh of **thermal input**. The model computes marginal cost per MWh electric as `Fuel_Cost / Efficiency`. For storage, `Efficiency` represents the round-trip efficiency; the model applies `sqrt(RTE)` symmetrically to each direction via PyPSA's `efficiency_store` and `efficiency_dispatch` parameters.

### `data/time_series/`

Each file is a single-column CSV with exactly **8,760 rows** (one per hour). No header row is required but is accepted.
...
| **3** | Configure technologies, objective, discount rate, currency, and demand scale |
| **4** | Load resource assessment CSV |
| **5** | Load hourly demand and generation time series |
| **6** | Build and solve the PyPSA optimisation model |
| **7** | Export results to Excel (`.xlsx`), dashboard JSON (`.json`), and economic CSV (`.csv`) |
...
| CO₂ summary | Per-technology and total annual emissions with intensity (gCO₂/kWh) and carbon tax liability |
| Grid congestion | Bus utilisation ratio timeseries, duration curve, and monthly congested hours breakdown |
| Excel export | Full 8,760-hour hourly dispatch including utilisation ratio and congestion flag |
| Dashboard JSON | Structured results file consumed by the interactive dashboard |
| Economic CSV | Flat per-technology export for downstream financial valuation (capacity, CF, CAPEX, OPEX, fuel cost) |
...
| `calculate_lcoe()` | Text | Per-technology LCOE/LCOS, system LCOE, and annual CO₂ summary with carbon tax |
...
### Dashboard JSON Structure (`dashboard_results.json`)

Produced by `model.export_dashboard_json(filepath, geo=geo)`. Pass `geo=geo` to embed geographic data in the output.

| Key | Contents |
|---|---|
| `meta` | Objective, discount rate, currency, grid loss factor, carbon tax rate, technology lists (all selected + optimiser-selected subsets) |
| `geographic` | Project name, latitude, longitude, max height, project description — only present when `geo` is passed |
| `capacities` | Installed MW (and MWh for storage) + `potential_mw` and `potential_energy_mwh` per technology |
...
| `lcoe_summary._system` | System LCOE, total annualised cost, total demand, `total_annualised_capex`, `total_annual_opex`, `total_annual_fuel`, `total_capex`, peak demand |
| `co2_summary` | Per-technology tCO₂/yr, total CO₂, emission intensity (gCO₂/kWh), total carbon tax liability, carbon tax rate |
| `brand_technologies` | Optional technology/model specification strings from resource assessment CSV |
...
### Economic CSV (`result_optimisation_economic.csv`)

Produced by `model.export_economic_csv(filepath)`. One row per technology plus a portfolio summary row.

| Column | Description |
|---|---|
| `Project_name` | Populated only in the first row; subsequent rows are blank for forward-fill compatibility |
| `Project_Description` | Project description from geographic data |
| `Currency` | Currency symbol from setup; first row only |
| `Discount_rate` | Discount rate applied to all technologies |
| `Lifetime` | Asset economic lifetime (years) |
| `Technology` | Technology name |
| `Capacity` | Installed power capacity (MW) |
| `Capacity_Factor` | Annual generation / (capacity × 8760) |
| `Energy_production` | Annual generation or discharge (MWh) |
| `Delivered_energy` | Energy after grid loss deduction (MWh) |
| `Curtailed_energy` | VRE curtailment (MWh); 0 for dispatchable and storage |
| `CAPEX` | Overnight capital cost (currency) |
| `OPEX` | Annual fixed O&M cost (currency/yr) |
| `Fuel_cost` | Annual variable fuel cost (currency/yr) |
| `Degradation_rate` | Placeholder (1); divide by 100 for % per year |
| `PPA_price` | Blank — fill before downstream analysis |
